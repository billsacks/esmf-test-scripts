...
class ArchiveResults:
        ...
        if not self._esmf_os:
            results = (
                subprocess.check_output(
                    f"grep ESMF_OS: {self.build_dir}/*_{self.data.job_id}.log",
                    shell=True,
                )
                .strip()
                .decode("utf-8")
    ...
    def build_succeeded(self) -> bool:
        if not self._build_succeeded:
            command = f"grep success {self.build_dir}/build_{self.data.job_id}.log"
            try:
    ...
    def monitor(self) -> None:
        ...
        while True:
            ...
            job_done = self.scheduler.check_queue(self.data.job_id)
            if job_done:
                oe_filelist = self._fetch_oe_filelist()
                logging.info("filelist is %s", oe_filelist)

                self._copy_artifacts(oe_filelist)
                break
            time.sleep(30)
    ...
    def _copy_artifacts(self, oe_filelist) -> None:
        logging.info(
    ...
    def _fetch_oe_filelist(self) -> List[Any]:
        oe_filelist = glob.glob(
            f"{self.data.test_root_dir}/{self.data.build_basename}/*_{self.data.job_id}*.log"
        )
        oe_filelist.extend(
    ...
    def _is_test_stage(self, oe_filelist) -> bool:
        for cfile in oe_filelist:
            ...
            if int(self.data.job_id) < 0:
                return True
            if (
                cfile.find(f"test_{self.data.job_id}") != -1
    ...
    def _generate_new_output_files(self, oe_filelist) -> None:
        for cfile in oe_filelist:
            nfile = os.path.basename(re.sub(f"_{self.data.job_id}", "", cfile))
            runcmd(f"echo `date` > {self.outpath}/out/{nfile}", self.is_dry_run)
            runcmd(f"cat {cfile} >> {self.outpath}/out/{nfile}", self.is_dry_run)
    ...
    def _run_final_stage(self) -> None:
        ...
        runcmd(";".join(git_cmd), self.is_dry_run)
    ...
    def _collect_example_artifacts(self) -> List[Any]:
        permutation = self.build_permutation
        return [
            *glob.glob(
    ...
    def _collect_example_results(self) -> str:
    ...
    def _collect_test_artifacts(self) -> List[Any]:
        permutation = self.build_permutation
        return [
    ...
    def _collect_unit_results(self) -> str:
        if self.build_failed:
            return "Build was not successful"
        permutation = self.build_permutation
    ...
    def _collect_system_results(self) -> str:
        if self.build_failed:
            return "Build was not successful"
        permutation = self.build_permutation
    ...
    def _collect_nuopc_results(self) -> Tuple:
        nuopc_pass = 0
        nuopc_fail = 0
        try:
            nuopc_pass = (
                subprocess.check_output(
                    f"grep PASS: {self.build_dir}/nuopc_{self.data.job_id}.log | wc -l",
                    shell=True,
                )
                .strip()
                .decode("utf-8")
            ...
            nuopc_fail = (
                subprocess.check_output(
                    f"grep FAIL: {self.build_dir}/nuopc_{self.data.job_id}.log | wc -l",
                    shell=True,
                )
                .strip()
                .decode("utf-8")
    ...
    def _collect_python_artifacts(self) -> List[Any]:
        return glob.glob(f"{self.build_dir}/src/addon/ESMPy/*.log")

    def _get_test_results(self) -> List[str]:
        results = []
        nuopc_pass, nuopc_fail = self._collect_nuopc_results()
        unit_results = re.sub(" FAIL", "\tFAIL", self._collect_unit_results())
        system_results = re.sub(" FAIL", " \tFAIL", self._collect_system_results())
        example_results = re.sub(" FAIL", " \tFAIL", self._collect_example_results())
    ...
    def _generate_artifact_files(self) -> None:
        ...
        example_artifacts = self._collect_example_artifacts()
        test_artifacts = self._collect_test_artifacts()
        python_artifacts = self._collect_python_artifacts()
